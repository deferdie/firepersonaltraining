...
const handleSelectConversation = (conv) => {
    router.visit(route('client.dashboard', { tab: 'chat', conversation: conv.id }), {
        preserveState: false,
    });
};
...
const handleSendMessage = (payload) => {
    ...
    router.post(route('client.messages.store'), {
...
watch(() => [activeTab.value, props.conversationId], () => {
    if (activeTab.value === 'chat' && props.conversationId) {
        subscribeToConversation(props.conversationId);
        axios.post(route('client.conversations.mark-read', props.conversationId)).then(() => {
            const convUnread = selectedConv.value?.unreadCount ?? 0;
            localUnreadCount.value = Math.max(0, (localUnreadCount.value ?? 0) - convUnread);
        }).catch(() => {});
    } else {
...
<template>
    ...
    >
        ...
        <div v-if="activeTab === 'chat'" class="space-y-4">
            ...
            <!-- Message Input -->
            <div v-if="props.conversationId" class="bg-white rounded-3xl p-4 shadow-sm border border-gray-100">
                <MessageComposer
