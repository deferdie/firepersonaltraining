<?php

namespace App\Http\Requests\Trainer;

use App\Enums\SchedulableType;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class StoreScheduleRequest extends FormRequest
{
    public function rules(): array
    {
        $type = SchedulableType::tryFrom($this->input('schedulable_type'));
        $table = $type?->tableName();

        $schedulableIdRules = ['required'];
        if ($table) {
            $schedulableIdRules[] = "exists:{$table},id";
        }

        return [
            'schedulable_type' => [
                'required',
                Rule::in(array_column(SchedulableType::cases(), 'value')),
            ],
            'schedulable_id' => $schedulableIdRules,
            'title' => ['required', 'string', 'max:255'],
            'notes' => ['nullable', 'string', 'max:2000'],
            'starts_at' => ['required', 'date'],
            'ends_at' => ['nullable', 'date', 'after_or_equal:starts_at'],
            'timezone' => ['nullable', 'string', 'max:64'],
            'recurrence_mode' => [
                'required',
                Rule::in(['one_off', 'daily', 'weekly', 'monthly', 'weekdays']),
            ],
            'recurrence_interval' => ['nullable', 'integer', 'min:1', 'max:365'],
            'recurrence_weekdays' => ['nullable', 'array'],
            'recurrence_weekdays.*' => ['integer', 'min:0', 'max:6'],
            'recurrence_ends_at' => ['nullable', 'date'],
        ];
    }
}
